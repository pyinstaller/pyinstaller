...
5. Start the Tcl interpreter and execute the splash screen
   ...
   environment variable ``_PYI_SPLASH_IPC``, which is also
...
is passed through the environment variable ``_PYI_SPLASH_IPC``. The bootloader
...
.. Note::
    Note that if the ``_PYI_SPLASH_IPC`` environment variable does not exist or an
