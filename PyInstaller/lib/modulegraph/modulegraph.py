    ...
    def _merged(self, other):
        if (not self.conditional and not self.function and not self.tryexcept) \
           or (not other.conditional and not other.function and not other.tryexcept):
            return DependencyInfo(conditional=False,
                                  function=False,
                                  tryexcept=False,
                                  fromlist=self.fromlist and other.fromlist)

        else:
            return DependencyInfo(conditional=self.conditional
...
class AliasNode(Node):
    ...
    def __init__(self, name, node):
        ...
        for attr_name in ('identifier', 'packagepath', '_global_attr_names',
                          '_starimported_ignored_module_names',
                          '_submodule_basename_to_node'):
...
class BaseModule(Node):
    ...
    def infoTuple(self):
        return tuple(
            filter(None, (self.identifier, self.filename, self.packagepath)))
    ...
    def _collect_import(self, name, fromlist, level):
        ...
        self._module._deferred_imports.append(
            (have_star, (name, self._module, fromlist, level), {
                'edge_attr':
                DependencyInfo(conditional=self.in_if,
...
class ModuleGraph(ObjectGraph):
    ...
    def _updateReference(self, fromnode, tonode, edge_data):
        ...
        if not (isinstance(ed, DependencyInfo)
                and isinstance(edge_data, DependencyInfo)):
    ...
    def add_edge(self, fromnode, tonode, edge_data='direct'):
        ...
        """
        return super(ModuleGraph, self).createReference(fromnode,
                                                        tonode,
                                                        edge_data=edge_data)
    ...
    def import_hook(
        ...
        """
        self.msg(3, "_import_hook", target_module_partname, source_module,
                 source_module, level)
        ...
        for target_module in target_modules:
            self._updateReference(source_module,
                                  target_module,
                                  edge_data=edge_attr)
    ...
    def _find_head_package(self,
        ...
        """
        self.msgin(4, "find_head_package", source_package,
                   target_module_partname, level)
        ...
        # Else if attempting only relative imports...
            ...
            for i in range(level - 1):
                ...
                assert new_parent is not source_package, (new_parent,
                                                          source_package)
        ...
        target_package = self._safe_import_module(target_module_headname,
                                                  target_package_name,
                                                  source_package)
        ...
        if target_package is not None:
            self.msgout(4, "find_head_package ->",
                        (target_package, target_module_tailname))
            return target_package, target_module_tailname

        # Else, raise an exception.
        self.msgout(4, "raise ImportError: No module named",
                    target_package_name)
    ...
    def _import_importable_package_submodules(self, package, attr_names):
        ...
        * Else, unignorable unimportable submodules, in which case an
        ...
        self.msgin(4, "_import_importable_package_submodules", package,
                   attr_names)
        ...
        for attr_name in attr_names:
            ...
            if submodule is None:
                ...
                submodule = self._safe_import_module(attr_name, submodule_name,
                                                     package)
    ...
    def alias_module(self, src_module_name, trg_module_name):
        ...
        self.msg(
            3,
            'alias_module "%s" -> "%s"' % (src_module_name, trg_module_name))
        # print('alias_module "%s" -> "%s"' % (src_module_name, trg_module_name))
        assert isinstance(
            src_module_name,
            str), '"%s" not a module name.' % str(src_module_name)
        assert isinstance(
            trg_module_name,
            str), '"%s" not a module name.' % str(trg_module_name)
        ...
        if trg_module is not None and not (isinstance(trg_module, AliasNode)
                                           and trg_module.identifier
                                           == src_module_name):
    ...
    def _safe_import_module(self, module_partname, module_name, parent_module):
        ...
        Node
            ...
            unimportable.
        """
        self.msgin(3, "safe_import_module", module_partname, module_name,
                   parent_module)
        ...
        if module is None:
            ...
            try:
                pathname, loader = self._find_module(module_partname,
                                                     search_dirs,
                                                     parent_module)
        ...
        if parent_module is not None:
            self.msg(4, "safe_import_module create reference", module, "->",
                     parent_module)

            # Add an edge from this submodule to its parent module.
            self._updateReference(module,
                                  parent_module,
        ...
        if loader is BUILTIN_MODULE:
            ...
            if src is not None:
                ...
                try:
                    ...
                    cls = (CompiledModule
                           if co is not None else InvalidCompiledModule)
                except Exception as exc:  # FIXME: more specific?
                    self.msg(
                        2, "load_module: InvalidCompiledModule, "
    ...
    def _safe_import_hook(self,
        ...
        """
        self.msg(3, "_safe_import_hook", target_module_partname, source_module,
                 target_attr_names, level)

        def is_swig_candidate():
            return (source_module is not None and target_attr_names is None
        ...
        try:
            target_modules = self.import_hook(target_module_partname,
                                              source_module,
                                              target_attr_names=None,
                                              level=level,
                                              edge_attr=edge_attr)
        ...
        # import schemes (namely, SWIG).
            ...
            if is_swig_candidate():
                self.msg(
                    4, 'SWIG import candidate (name=%r, caller=%r, level=%r)' %
                ...
                if is_swig_import:
                    ...
                    level = 1
                    self.msg(
                        2, 'SWIG import (caller=%r, fromlist=%r, level=%r)' %
        ...
        if isinstance(target_module, MissingModule) \
           ...
           and is_swig_wrapper(source_module):
            ...
            target_modules = self._safe_import_hook(target_module_partname,
                                                    source_module,
                                                    target_attr_names=None,
                                                    level=1,
                                                    edge_attr=edge_attr)
        ...
        if target_attr_names and isinstance(target_module,
            ...
            for target_submodule_partname in target_attr_names:
                ...
                # Fully-qualified name of this submodule.
                target_submodule_name = (target_module.identifier + '.' +
                                         target_submodule_partname)
                ...
                if target_submodule is None:
                    ...
                    try:
                        ...
                        target_submodule = self.find_node(
                        ...
                        if is_swig_import:
                            ...
                            if self.find_node(target_submodule_partname):
                                self.msg(
                                    2, 'SWIG import error: %r basename %r '
                                    'already exists' %
                                    (target_submodule_name,
                                     target_submodule_partname))
                            else:
                                self.msg(
                                    4, 'SWIG import renamed from %r to %r' %
                                    (target_submodule_name,
                                     target_submodule_partname))
                ...
                target_module.add_submodule(target_submodule_partname,
                                            target_submodule)
                if target_submodule is not None:
                    self._updateReference(target_module,
                                          target_submodule,
                                          edge_data=edge_attr)
                    self._updateReference(source_module,
                                          target_submodule,
    ...
    def _scan_code(self,
        ...
        **This method is at the root of all `ModuleGraph` recursion.**
        ...
        if module_code_object_ast is not None:
            ...
            self._scan_bytecode(module,
                                module_code_object,
                                is_scanning_imports=False)
        # Else, parse this module's code object for imports.
        else:
            self._scan_bytecode(module,
                                module_code_object,
                                is_scanning_imports=True)
        ...
        for inst in util.iterate_instructions(module_code_object):
            ...
            if inst.opname == 'IMPORT_NAME':
                ...
                module._deferred_imports.append(
                    (have_star, (target_module_partname, module, fromlist,
                                 level), {}))
        ...
        try:
            for search_dir in search_dirs:
                ...
                # Else, raise an exception.
                    ...
                    raise ImportError("Module %r loader %r path unobtainable" %
                                      (module_name, loader))
    ...
    def itergraphreport(self, name='G', flatpackages=()):
        ...
        for (node, data, outgoing, incoming) in nodes:
            ...
            yield '\t"%s" [%s];\n' % (
                node,
                ','.join([(cpatt % item) for item in nodevisitor(
                    node, data, outgoing, incoming).items()]),
            )
    ...
    def report(self):
        ...
        for m in sorted(self.iter_graph(), key=lambda n: n.identifier):
            print("%-15s %-25s %s" %
                  (type(m).__name__, m.identifier, m.filename or ""))
    ...
    def _replace_paths_in_code(self, co):
        ...
        if hasattr(co, 'replace'):  # is_py38
            ...
            return code_func(co.co_argcount, co.co_kwonlyargcount,
                             co.co_nlocals,
            ...
            return code_func(co.co_argcount, co.co_nlocals,
                             co.co_stacksize, co.co_flags, co.co_code,
                             tuple(consts), co.co_names, co.co_varnames,
                             new_filename, co.co_name, co.co_firstlineno,
                             co.co_lnotab, co.co_freevars, co.co_cellvars)
