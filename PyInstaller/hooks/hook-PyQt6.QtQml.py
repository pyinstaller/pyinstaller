...
from PyInstaller.utils.hooks.qt import pyqt6_library_info, add_qt6_dependencies
...
if pyqt6_library_info.version is not None:
    ...
    qmldir = pyqt6_library_info.location['Qml2ImportsPath']
    ...
    if not os.path.exists(qmldir):
        logger.warning('Unable to find Qt6 QML files. QML files not packaged.')
    else:
        qml_rel_dir = ['PyQt6', 'Qt', 'qml']
