...
if pyqt6_library_info.version is not None:
    ...
    rel_data_path = ['PyQt6', 'Qt']
    if compat.is_darwin:
        # This is based on the layout of the Mac wheel from PyPi.
        data_path = pyqt6_library_info.location['DataPath']
        ...
        datas += [
            ...
            (os.path.join(pyqt6_library_info.location['TranslationsPath'],
                          locales),
             os.path.join('PyQt6', 'Qt', 'translations', locales)),
            ...
            (os.path.join(pyqt6_library_info.location['DataPath'], resources),
             os.path.join(*(rel_data_path + [resources]))),
            ...
            (os.path.join(pyqt6_library_info.location['LibraryExecutablesPath'],
                          'QtWebEngineProcess*'),
             os.path.join(*(rel_data_path +
                          [remove_prefix(pyqt6_library_info.location['LibraryExecutablesPath'],
                                        pyqt6_library_info.location['PrefixPath'] + '/')])))
        ]
    ...
    if compat.is_linux:
        ...
        for imp in getImports(get_module_file_attribute('PyQt6.QtWebEngineWidgets')):
            # Look for ``libnss3.so``.
