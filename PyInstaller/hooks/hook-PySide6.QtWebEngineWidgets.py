...
if pyside6_library_info.version:
    ...
    if compat.is_win:
        rel_data_path = ['PySide6']
    else:
        rel_data_path = ['PySide6', 'Qt']

    pyside6_locations = pyside6_library_info.location
    if compat.is_darwin:
        # This is based on the layout of the Mac wheel from PyPi.
        data_path = pyside6_locations['DataPath']
        ...
        datas += [
            ...
            (os.path.join(pyside6_locations['TranslationsPath'], locales),
            ...
            #
            (os.path.join(pyside6_locations['DataPath'], resources),
            ...
            #
            (os.path.join(pyside6_locations['LibraryExecutablesPath'],
                          'QtWebEngineProcess*'),
             prefix_with_path(rel_data_path, get_relative_path_if_possible(
                 pyside6_locations['LibraryExecutablesPath'],
                 pyside6_locations['PrefixPath'] + '/')))
        ]
    ...
    if compat.is_linux:
        ...
        for imp in getImports(
                get_module_file_attribute('PySide6.QtWebEngineWidgets')):
            # Look for ``libnss3.so``.
