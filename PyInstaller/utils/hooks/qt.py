...
Qt_Libraries = ('PyQt5', 'PySide2', 'PyQt6')
            ...
            json_str = exec_statement("""
                ...
                try:
                    ...
                    if hasattr(QLibraryInfo, 'path'):
                        paths = [x for x in dir(QLibraryInfo.LibraryPath) if x.endswith('Path')]
                        location = {x: QLibraryInfo.path(getattr(QLibraryInfo.LibraryPath, x))
                                    for x in paths}
                    else:
...
def qt_plugins_dir(namespace):
    ...
    if namespace == 'PyQt5':
        ...
        paths = [pyqt6_library_info.location['PluginsPath']]
    else:
...
def qt_plugins_binaries(plugin_type, namespace):
    ...
    if namespace == 'PyQt5':
        ...
        plugin_dir = os.path.join('PyQt6', 'Qt', 'plugins')
...
# https://code.qt.io/cgit/qt/qttools.git/tree/src/windeployqt/main.cpp?h=v6.0.0
_qt6_dynamic_dependencies_dict = {
    ## "lib_name":              (.hiddenimports,           translations_base,  zero or more plugins...)
    "qt6bluetooth":             (".QtBluetooth",           None,               ),  # noqa: E241,E202
    "qt6concurrent":            (None,                     "qtbase",           ),
    "qt6core":                  (".QtCore",                "qtbase",           ),
    # This entry generated by hand -- it's not present in the Windows deployment tool sources.
    "qtdbus":                   (".QtDBus",                None,               ),
    "qt6declarative":           (None,                     "qtquick1",         "qml1tooling"),
    "qt6designer":              (".QtDesigner",            None,               ),
    "qt6designercomponents":    (None,                     None,               ),
    "qt6gamepad":               (None,                     None,               "gamepads"),
    # Note: The ``platformthemes`` plugin is for Linux only, and comes from earlier PyInstaller code in ``hook-PyQt5.QtGui.py``. The ``styles`` plugin comes from the suggestion at https://github.com/pyinstaller/pyinstaller/issues/2156.
    # ``xcbglintegrations`` and ``egldeviceintegrations`` were added manually
    # for linux
    "qt6gui":                   (".QtGui",                 "qtbase",           "accessible", "iconengines", "imageformats", "platforms", "platforminputcontexts", "virtualkeyboard", "platformthemes", "styles", "xcbglintegrations", "egldeviceintegrations"),  # noqa
    "qt6help":                  (".QtHelp",                "qt_help",          ),
    # This entry generated by hand -- it's not present in the Windows deployment tool sources.
    "qt6macextras":             (".QtMacExtras",           None,               ),
    "qt6multimedia":            (".QtMultimedia",          "qtmultimedia",     "audio", "mediaservice", "playlistformats"),
    "qt6multimediawidgets":     (".QtMultimediaWidgets",   "qtmultimedia",     ),
    "qt6multimediaquick_p":     (None,                     "qtmultimedia",     ),
    "qt6network":               (".QtNetwork",             "qtbase",           "bearer"),
    "qt6nfc":                   (".QtNfc",                 None,               ),
    "qt6opengl":                (".QtOpenGL",              None,               ),  # noqa
    "qt6openglwidgets":         (".QtOpenGLWidgets",       None,               ),
    "qt6positioning":           (".QtPositioning",         None,               "position"),
    "qt6printsupport":          (".QtPrintSupport",        None,               "printsupport"),
    "qt6qml":                   (".QtQml",                 "qtdeclarative",    ),
    "qmltooling":               (None,                     None,               "qmltooling"),
    "qt6quick":                 (".QtQuick",               "qtdeclarative",    "scenegraph", "qmltooling"),  # noqa
    "qt6quickparticles":        (None,                     None,               ),
    "qt6quickwidgets":          (".QtQuickWidgets",        None,               ),
    "qt6script":                (None,                     "qtscript",         ),
    "qt6scripttools":           (None,                     "qtscript",         ),
    "qt6sensors":               (".QtSensors",             None,               "sensors", "sensorgestures"),
    "qt6serialport":            (".QtSerialPort",          "qtserialport",     ),
    "qt6sql":                   (".QtSql",                 "qtbase",           "sqldrivers"),
    "qt6svg":                   (".QtSvg",                 None,               ),
    "qt6svgwidgets":            (".QtSvgWidgets",          None,               ),
    "qt6test":                  (".QtTest",                "qtbase",           ),
    "qt6websockets":            (".QtWebSockets",          None,               ),
    "qt6widgets":               (".QtWidgets",             "qtbase",           ),
    "qt6winextras":             (".QtWinExtras",           None,               ),
    "qt6xml":                   (".QtXml",                 "qtbase",           ),
    "qt6webenginecore":         (".QtWebEngineCore",       None,               "qtwebengine"),  # noqa
    "qt6webengine":             (".QtWebEngine",           "qtwebengine",      "qtwebengine"),
    "qt6webenginewidgets":      (".QtWebEngineWidgets",    None,               "qtwebengine"),
    "qt63dcore":                (None,                     None,               ),
    "qt63drender":              (None,                     None,               "sceneparsers", "renderers", "renderplugins", "geometryloaders"),
    "qt63dquick":               (None,                     None,               ),
    "qt63dquickRender":         (None,                     None,               ),
    "qt63dinput":               (None,                     None,               ),
    "qt63danimation":           (None,                     None,               ),
    "qt63dextras":              (None,                     None,               ),
    "qt6location":              (".QtLocation",            None,               "geoservices"),
    "qt6webchannel":            (".QtWebChannel",          None,               ),
    "qt6texttospeech":          (None,                     None,               "texttospeech"),
    "qt6serialbus":             (None,                     None,               "canbus"),
    "qt6webview":               (None,                     None,               "webview"),
}
...
def add_qt5_dependencies(hook_file):
    ...
    while imports:
        ...
        if lib_name in _qt5_dynamic_dependencies_dict:
            ...
            dd = _qt5_dynamic_dependencies_dict[lib_name]
...
def add_qt6_dependencies(hook_file):
    # Accumulate all dependencies in a set to avoid duplicates.
    hiddenimports = set()
    translations_base = set()
    plugins = set()
    ...
    # Find the module underlying this Qt hook: change
    # ``/path/to/hook-PyQt6.blah.py`` to ``PyQt6.blah``.
    hook_name, hook_ext = os.path.splitext(os.path.basename(hook_file))
    assert hook_ext.startswith('.py')
    assert hook_name.startswith('hook-')
    module_name = hook_name[5:]
    ...
    if namespace not in Qt_Libraries:
        raise Exception('Invalid namespace: {0}'.format(namespace))
    is_PyQt6 = namespace == 'PyQt6'

    # Exit if the requested library can't be imported.
    if is_PyQt6 and not pyqt6_library_info.version:
        return [], [], []

    # Look up the module returned by this import.
    module = get_module_file_attribute(module_name)
    logger.debug('add_qt6_dependencies: Examining %s, based on hook of %s.',
                 module, hook_file)
    ...
    while imports:
        ...
        if is_win:
            imp = getfullnameof(imp,
                # First, look for Qt binaries in the local Qt install.
                pyqt6_library_info.location['BinariesPath']
            )
        ...
        if lib_name.startswith('lib'):
            lib_name = lib_name[3:]
        # Mac: rename from ``qt`` to ``qt6`` to match names in Windows/Linux.
        if is_darwin and lib_name.startswith('qt'):
            lib_name = 'qt6' + lib_name[2:]

        # match libs with QT_LIBINFIX set to '_conda', i.e. conda-forge builds
        if lib_name.endswith('_conda'):
            lib_name = lib_name[:-6]

        logger.debug('add_qt6_dependencies: raw lib %s -> parsed lib %s',
                     imp, lib_name)

        # Follow only Qt dependencies.
        if lib_name in _qt6_dynamic_dependencies_dict:
            # Follow these to find additional dependencies.
            logger.debug('add_qt6_dependencies: Import of %s.', imp)
            imports.update(getImports(imp))
            # Look up which plugins and translations are needed.
            dd = _qt6_dynamic_dependencies_dict[lib_name]
            lib_name_hiddenimports, lib_name_translations_base = dd[:2]
            lib_name_plugins = dd[2:]
            # Add them in.
            if lib_name_hiddenimports:
    ...
    # Change translation_base to datas.
    tp = (
        pyqt6_library_info.location['TranslationsPath']
    )
    datas = []
    for tb in translations_base:
        src = os.path.join(tp, tb + '_*.qm')
        ...
        if glob.glob(src):
            datas.append((
                src, os.path.join(
                    # The PySide2 Windows wheels place translations in a
                    # different location.
                    namespace, '' if not is_PyQt6 and is_win else 'Qt',
                    'translations'
                )
            ))
        else:
            logger.warning('Unable to find Qt6 translations %s. These '
                           'translations were not packaged.', src)
    # Change hiddenimports to a list.
    hiddenimports = list(hiddenimports)

    logger.debug('add_qt6_dependencies: imports from %s:\n'
                 '  hiddenimports = %s\n'
                 '  binaries = %s\n'
                 '  datas = %s',
                 hook_name, hiddenimports, binaries, datas)
