    ...
    def __getattr__(self, name):
        if 'version' in self.__dict__:
            ...
            try:
                ...
                logger.warning(
                    'Cannot read QLibraryInfo output: raised %s when '
                    'decoding:\n%s', str(e), json_str)
                qli = {}
...
_qt5_dynamic_dependencies_dict = {
    ...
    # for linux
    "qt5gui": (".QtGui", "qtbase", "accessible", "iconengines", "imageformats",
               "platforms", "platforminputcontexts", "platformthemes",
               "styles", "xcbglintegrations", "egldeviceintegrations"),  # noqa
    "qt5help": (
        ".QtHelp",
        "qt_help",
    ),
    ...
    "qt5multimedia": (".QtMultimedia", "qtmultimedia", "audio", "mediaservice",
                      "playlistformats"),
    ...
    "qt5quick":
    (".QtQuick", "qtdeclarative", "scenegraph", "qmltooling"),  # noqa
    "qt5quickparticles": (
        None,
        None,
    ),
    ...
    "qt53drender":
    (None, None, "sceneparsers", "renderplugins", "geometryloaders"),
    "qt53dquick": (
        None,
        None,
    ),
...
def _find_all_or_none(globs_to_include, num_files, qt_library_info):
    ...
    for dll in globs_to_include:
        ...
        dll_path = os.path.join(
            qt_library_info.location['BinariesPath' if qt_library_info.
                                     is_pyqt else 'PrefixPath'], dll)
...
def get_qt_conf_file(qt_library_info):
    ...
    datas = [
        x for x in hooks.collect_system_data_files(
            qt_library_info.location['PrefixPath'], qt_library_info.qt_rel_dir)
...
def get_qt_webengine_binaries_and_data_files(qt_library_info):
    ...
    if compat.is_darwin:
        ...
        datas += [
            ...
            (os.path.join(qt_library_info.location['DataPath'],
                          resources), os.path.join(rel_data_path, resources)),
