...
class VSVersionInfo:
    ...
    def toRaw(self):
        ...
        return (struct.pack('hhh', sublen, vallen, typ) + raw_name +
                b'\000\000' + pad + rawffi + pad2 + tmp)
...
class FixedFileInfo:
    ...
    def fromRaw(self, data, i):
        (self.sig, self.strucVersion, self.fileVersionMS, self.fileVersionLS,
         self.productVersionMS, self.productVersionLS, self.fileFlagsMask,
         self.fileFlags, self.fileOS, self.fileType, self.fileSubtype,
         self.fileDateMS,
         self.fileDateLS) = struct.unpack('13L', data[i:i + 52])
        return i + 52

    def toRaw(self):
        return struct.pack('13L', self.sig, self.strucVersion,
                           self.fileVersionMS, self.fileVersionLS,
                           self.productVersionMS, self.productVersionLS,
                           self.fileFlagsMask, self.fileFlags, self.fileOS,
                           self.fileType, self.fileSubtype, self.fileDateMS,
                           self.fileDateLS)
    ...
    def __str__(self, indent=u''):
        ...
        tmp = [
            ...
            u'OS=%s,' % hex(self.fileOS), u'# The general type of file.',
    ...
    def __repr__(self):
        ...
        return ('versioninfo.FixedFileInfo(filevers=%r, prodvers=%r, '
                ...
                (fv, pv, self.fileFlagsMask, self.fileFlags, self.fileOS,
...
class StringTable:
    ...
    def fromRaw(self, data, i, limit):
        i, (cpsublen, cpwValueLength, cpwType,
            self.name) = parseCodePage(data, i,
                                       limit)  # should be code page junk
        i = nextDWord(i)
        while i < limit:
            ss = StringStruct()
...
class StringStruct:
    ...
    def toRaw(self):
        ...
        if sublen % 4:
            pad = b'\000\000'
        sublen = sublen + len(pad) + vallen
        abcd = (struct.pack('hhh', sublen, vallen, typ) + raw_name +
                b'\000\000' + pad + raw_val + b'\000\000')
        return abcd
...
class VarFileInfo:
    ...
    def __str__(self, indent=''):
        return (indent +
                "VarFileInfo([%s])" % ', '.join(str(kid) for kid in self.kids))
...
class VarStruct:
    ...
    def fromRaw(self, data, i, limit):
        i, (self.sublen, self.wValueLength, self.wType,
            self.name) = parseCommon(data, i)
        i = nextDWord(i)
