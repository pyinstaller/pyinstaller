...
class ModuleHook(object):
    ...
    def __init__(self, module_graph, module_name, hook_filename,
        ...
        self.hook_module_name = (hook_module_name_prefix +
    ...
    def _process_hook_func(self, analysis):
        ...
        hook_api = PostGraphAPI(module_name=self.module_name,
                                module_graph=self.module_graph,
                                analysis=analysis)
        try:
    ...
    def _process_hidden_imports(self):
        ...
        for import_module_name in self.hiddenimports:
            try:
                ...
                caller = self.module_graph.find_node(self.module_name,
                                                     create_nspkg=False)
        ...
        for item in set(self.excludedimports):
            ...
            for src in hooked_mods:
                ...
                for dest in imports_to_remove & references:
                    ...
                    logger.debug("Excluding import of %s from module %s", dest,
                                 src)
