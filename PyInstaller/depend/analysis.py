...
from PyInstaller.compat import (BAD_MODULE_TYPES, BINARY_MODULE_TYPES,
                                MODULE_TYPES_TO_TOC_DICT,
                                PURE_PYTHON_MODULE_TYPES, PY3_BASE_MODULES,
...
from PyInstaller.depend.imphookapi import (PreFindModulePathAPI,
...
class PyiModuleGraph(ModuleGraph):
    ...
    def _reset(self, user_hook_dirs):
        ...
        self._user_hook_dirs = (list(user_hook_dirs) +
                                [os.path.join(PACKAGEPATH, 'hooks')])
        ...
        self._hooks_pre_safe_import_module = self._cache_hooks(
            'pre_safe_import_module')
        self._hooks_pre_find_module_path = self._cache_hooks(
    ...
    def _merge_rthooks(self, rthooks, uhd, uhd_path):
        ...
        assert isinstance(rthooks,
                          dict), ('The root element in %s must be a dict.' %
                                  uhd_path)
        for module_name, python_file_name_list in rthooks.items():
            ...
            assert isinstance(
                python_file_name_list,
                list), ('The value of %s key %s must be a list.' %
    ...
    def msg(self, level, s, *args):
        ...
        record = logger.makeRecord(logger.name, level, fn, lno, msg, [], None,
                                   func, None, sinfo)
    ...
    def add_script(self, pathname, caller=None):
        ...
        if self._top_script_node is None:
            ...
            try:
                self._top_script_node = super(PyiModuleGraph,
                                              self).add_script(pathname)
            except SyntaxError:
        ...
        while True:
            ...
            for module_name, module_hooks in self._hooks.items():
                ...
                for module_hook in module_hooks:
                    ...
                    self._additional_files_cache.add(module_name,
    ...
    def get_importers(self, name):
        ...
        importers = (importer.identifier for importer in importers
...
def get_bootstrap_modules():
    ...
    loader_mods += [
        ...
        ('pyimod01_os_path', os.path.join(loaderpath,
                                          'pyimod01_os_path.pyc'), 'PYMODULE'),
        ('pyimod02_archive', os.path.join(loaderpath,
                                          'pyimod02_archive.pyc'), 'PYMODULE'),
        ('pyimod03_importers',
         os.path.join(loaderpath, 'pyimod03_importers.pyc'), 'PYMODULE'),
        ('pyimod04_ctypes', os.path.join(loaderpath, 'pyimod04_ctypes.pyc'),
         'PYMODULE'),  # noqa: E501
        ('pyiboot01_bootstrap',
