...
def load_ldconfig_cache():
    ...
    if is_freebsd or is_openbsd:
        ...
        # Example linux output with local cs_CZ.UTF-8:
        #$ /sbin/ldconfig -p
    ...
    ldconfig_pattern = r'^[ \t]+(?P<_bsd>\d+:-)?'
    ldconfig_pattern += r'(?P<name>l.+?)'
    ldconfig_pattern += r'(?P<_info>[ \t]+\(.*\))? => '
    ldconfig_pattern += r'(?P<path>.+)(?=$)'
    pattern = re.compile(ldconfig_pattern, re.M)
    try:
    ...
    for match in pattern.finditer(text):
        ...
        if is_freebsd or is_openbsd:
            ...
            name = 'lib' + name[1:]  # skip starting l
