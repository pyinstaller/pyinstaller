...
class PKG(Target):
    ...
    def assemble(self):
        ...
        archive = CArchiveWriter(self.name, srctoc + mytoc, pylib_name=pylib_name)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if is_win:
            if not self.icon:
                ...
                self.icon = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'bootloader', 'images', icon)
        ...
        if not self.append_pkg:
            ...
            if pylib_version < exe_version:
                logger.info(
                    ...
                    "issues in the frozen application.", *exe_version, *pylib_version
                )
                ...
                if not tpl[1] in self._dependencies:
                    ...
                    if dep_path.endswith(path):
                        logger.debug(
                            ...
                            "located in %s - duplicated TOC entry?", tpl[1], path, dep_path
                        )
