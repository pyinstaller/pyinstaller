...
from PyInstaller.building.utils import (_check_guts_toc, _make_clean_directory,
                                        add_suffix_to_extension, checkCache,
                                        get_code_object, strip_paths_in_code)
from PyInstaller.compat import (exec_command_all, is_cygwin,
                                is_darwin, is_linux, is_win)
...
if is_win:
    from PyInstaller.utils.win32 import (icon, versioninfo, winmanifest,
                                         winresource)
                ...
                if not tpl[1] in self._dependencies:
                    ...
                    if dep_path.endswith(path):
                        logger.debug(
                            ...
                            "located in %s - duplicated TOC entry?", tpl[1],
                            path, dep_path)
                    ...
                    analysis.dependencies.append((":".join(
                        (dep_path, filename)), tpl[1], "DEPENDENCY"))
                    toc[i] = (None, None, None)
            # Clean the list
