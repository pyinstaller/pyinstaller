...
class PYZ(Target):
    ...
    def __init__(self, *tocs, **kwargs):
        ...
        self.cipher = cipher
        if cipher:
            key_file = ('pyimod00_crypto_key', os.path.join(
                CONF['workpath'], 'pyimod00_crypto_key.pyc'), 'PYMODULE')
...
class PKG(Target):
    ...
    def assemble(self):
                ...
                if self.exclude_binaries and typ != 'DEPENDENCY':
                    ...
                    if typ == 'BINARY':
                        ...
                        if fnm in seenFnms:
                            ...
                            logger.warning('was placed previously at')
                            logger.warning(
                                pprint.pformat((seenFnms[fnm], fnm,
                                                seenFnms_typ[fnm])))
                    seenInms[inm] = fnm
                    seenFnms[fnm] = inm
        ...
        archive = CArchiveWriter(
            self.name, srctoc + mytoc, pylib_name=pylib_name)
...
class EXE(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.runtime_tmpdir is not None:
            self.toc.append(("pyi-runtime-tmpdir " + self.runtime_tmpdir, "",
                             "OPTION"))
        ...
        exe = self._bootloader_file('run', '.exe'
                                    if is_win or is_cygwin else '')
...
class MERGE(object):
    ...
    def __init__(self, *args):
        ...
        common_prefix = os.path.commonprefix([
            os.path.normcase(os.path.abspath(a.scripts[-1][1]))
            for a, _, _ in args
        ])
    ...
    def _merge_dependencies(self, args):
        ...
        for analysis, _, _ in args:
            path = os.path.abspath(analysis.scripts[-1][1]).replace(
                self._common_prefix, "", 1)
        ...
        for toc in (analysis.binaries, analysis.datas):
            for i, tpl in enumerate(toc):
                if not tpl[1] in self._dependencies:
                    ...
                    logger.debug(
                        "Referencing %s to be a dependecy for %s, located in %s"
                        % (tpl[1], path, dep_path))
                    analysis.dependencies.append((":".join((dep_path, tpl[0])),
                                                  tpl[1], "DEPENDENCY"))
