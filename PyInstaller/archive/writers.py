...
from PyInstaller.building.utils import (fake_pyc_timestamp, get_code_object,
                                        strip_paths_in_code)
from PyInstaller.compat import BYTECODE_MAGIC, is_py37, is_win
from PyInstaller.loader.pyimod02_archive import (PYZ_TYPE_DATA,
                                                 PYZ_TYPE_MODULE,
                                                 PYZ_TYPE_NSPKG, PYZ_TYPE_PKG)
    ...
    def tobinary(self):
        ...
        for (dpos, dlen, ulen, flag, typcd, nm) in self.data:
            ...
            if toclen % 16 == 0:
                ...
                nmlen = nmlen + padlen
            rslt.append(
                struct.pack(self.ENTRYSTRUCT + '%is' % nmlen,
                            nmlen + self.ENTRYLEN, dpos, dlen, ulen, flag,
                            ord(typcd), nm + pad))
...
class SplashWriter(ArchiveWriter):
    ...
    def __init__(self, archive_path, name_list, tcl_libname, tk_libname, tklib,
                 rundir, image, script):
        """
    ...
    def update_headers(self, tocpos):
        ...
        self.lib.seek(self.start)
        self.lib.write(
            struct.pack(self._header_format, self._tcl_libname.encode("utf-8"),
                        self._tk_libname.encode("utf-8"),
                        self._tklib.encode("utf-8"),
                        self._rundir.encode("utf-8"), self._script_len,
                        self._script_offset, self._image_len,
                        self._image_offset, self._requirements_len,
