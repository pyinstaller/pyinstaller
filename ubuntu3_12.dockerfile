...
#
#   docker build -f ubuntu3_12.dockerfile -t ubuntu3_12 .
#   docker run -it ubuntu3_12
#   pytest
#
# Or if you prefer a one-liner:
#
#   docker run -it $(docker build -q -f ubuntu3_12.dockerfile .) pytest
#
...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y python3.12 python3.12-dev python3.12-distutils
...
RUN python3.12 setup.py -qqq bdist_wheel -d wheels
