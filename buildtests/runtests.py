...
class TestRunner(object):
    ...
    def run(test_name, verbose=False):
        ...
        if not os.path.exists(test_name + '.py'):
            _msg("Testfile not found:", test_name + '.py', short=1)
        ...
        if res == 0:
            _msg("FINISHING TEST", test_name, short=1)
            counter["passed"].append(test_name)
        else:
            _msg("TEST", test_name, "FAILED", short=1, sep="!!")
            counter["failed"].append(test_name)
...
def generic_test_function(test_name):
    ...
    run_test([test_name + '.py'], verbose=True)
