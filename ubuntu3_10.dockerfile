...
#
#   docker build -f ubuntu3_10.dockerfile -t ubuntu3_10 .
#   docker run -it ubuntu3_10
#   pytest
#
# Or if you prefer a one-liner:
#
#   docker run -it $(docker build -q -f ubuntu3_10.dockerfile .) pytest
#
...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y python3.10 python3.10-dev python3.10-distutils
RUN echo '#!/bin/bash\npython3.10' > /usr/bin/python && \
...
RUN python3.10 setup.py -qqq bdist_wheel -d wheels
