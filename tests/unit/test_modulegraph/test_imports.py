...
class TestModuleGraphImport(unittest.TestCase):
    ...
    def testGraphStructure(self):
        ...
        self.assertEqual(
            set(refs), set(self.mf.findNode(nm) for nm in imported))
    ...
    if sys.version_info[0] == 2:
        ...
        def testOldStyle(self):
            ...
            sub = [
                n for n in self.mf.get_edges(node)[0]
                if n.identifier != '__future__'
            ][0]
    ...
    def testNewStyle(self):
        ...
        sub = [
            n for n in self.mf.get_edges(node)[0]
            if not n.identifier.startswith('__future__')
        ][0]
        self.assertEqual(sub.identifier, 'mod')
...
class TestRegressions1(unittest.TestCase):
    ...
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr1')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
    ...
    def testMissingPathEntry(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'nosuchdirectory')
        try:
...
class TestRegressions2(unittest.TestCase):
    ...
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr2')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
        self.mf.run_script(os.path.join(root, 'main_script.py'))
...
class TestRegressions3(unittest.TestCase):
    ...
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr3')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
        self.mf.run_script(os.path.join(root, 'script.py'))

    @unittest.skipUnless(not hasattr(sys, 'real_prefix'),
                         "Test doesn't work in virtualenv")
    def testRegr1(self):
...
class TestRegression4(unittest.TestCase):
    ...
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr4')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
        self.mf.run_script(os.path.join(root, 'script.py'))
...
class TestRegression5(unittest.TestCase):
    ...
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr5')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
        self.mf.run_script(os.path.join(root, 'script.py'))
...
class TestDeeplyNested(unittest.TestCase):
    def setUp(self):
        root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'testpkg-regr6')
        self.mf = modulegraph.ModuleGraph(path=[root] + sys.path)
        self.mf.run_script(os.path.join(root, 'script.py'))
