...
class BuildTestRunner(object):
    ...
    def _find_exepath(self, test, parent_dir='dist'):
        ...
        searching for each of 'test_multipackage1*' and
        'multipackage1*' in one-file- and one-dir-mode.
        """
        ...
        patterns = [
            ...
            os.path.join(parent_dir, test, test),
            # search for e.g. `multipackage2_B`, too:
            os.path.join(parent_dir, name),
            os.path.join(parent_dir, name, name),
            ]
    ...
    def test_exe(self):
        ...
        found = False
        retcode = 0
        for exe in self._find_exepath(self.test_file, 'dist'):
            found = True
            retcode_tmp = self._run_created_exe(exe)
            retcode = retcode or retcode_tmp
        if not found:
            self._plain_msg('ERROR: no file generated by PyInstaller found!')
            return 1, list(self._find_exepath(self.test_file, 'dist'))
        return retcode == 0
